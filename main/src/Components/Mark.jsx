  ...
  useEffect(() => {
    if (selectedStudent) {
      ...
      if (student) {
        ...
        // Load student's existing marks
        const initialMarks = {};
        student.subjects?.forEach((subject) => {
        initialMarks[subject.name] = subject.mark;
        });
          ...
          <form onSubmit={handleSubmit}>
            <div className='mark-btn-onside'>
              ...
              <table className='mark-table'>
                ...
                <tbody>
                  {students.find(stud => stud._id === selectedStudent)?.subjects.length > 0 ? 
                    students.find(stud => stud._id === selectedStudent)?.subjects?.map((subject) => (
                      <tr key={subject._id}>
                        <td>{subject.name}</td>
                        <td>
                          <input
                      ...
                      <tr key={subject}>
                        ...
                        <td>
                          <input
                            type="number"
                            placeholder="Enter Marks"
                            defaultValue={marks[subject] ? marks[subject] : 0}
...
//         const calculatedPercentage = (totalMarks / (student.subjects.length * 100)) * 100;
...
//     setMarks((prev) => {
//       const updatedMarks = { ...prev, [subject]: markValue };

//       const totalMarks = Object.values(updatedMarks).reduce((acc, m) => acc + m, 0);
//       const calculatedPercentage = (totalMarks / (Object.keys(updatedMarks).length * 100)) * 100;
//       setPercentage(calculatedPercentage.toFixed(2));
...
//                 (dept) => dept._id === students.find((s) => s._id === selectedStudent)?.department?._id
//               )?.department || ''}
//               disabled
//             />
//           </div>
